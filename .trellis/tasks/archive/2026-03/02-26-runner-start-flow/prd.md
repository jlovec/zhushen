# hapi runner start 流程分析

## 目标

理解在 Windows PowerShell 7 终端中执行 `hapi runner start .` 后的完整执行流程，包括：
- 命令执行链路
- Claude Code 会话启动过程
- MCP 服务器启动机制
- 各个进程在哪个终端中运行
...
- **命令示例**：`hapi claude --hapi-starting-mode remote --started-by runner`
...
#### HAPI MCP Server（每个会话独立）
- **启动位置**：`cli/src/claude/runClaude.ts` 中的 `startHappyServer()`
...
#### MCP STDIO Bridge（可选）
- **命令**：`hapi mcp --url <http://127.0.0.1:PORT>`
- **作用**：将 STDIO MCP 协议桥接到 HTTP MCP 服务器
- **使用场景**：当 Claude Code 需要通过 STDIO 协议访问 HAPI MCP 工具时
- **实现**：`cli/src/codex/happyMcpStdioBridge.ts`
...
```
PowerShell 终端 (用户执行 hapi runner start)
│
├─ hapi runner start (立即退出)
│   └─ spawnHappyCLI(['runner', 'start-sync'], { detached: true, stdio: 'ignore' })
│       └─ unref() - 解除父进程引用
│
└─ Runner 进程 (后台守护进程，detached)
    ...
    │
    └─ 会话进程们 (每个都是 detached)
        ├─ Session 1: hapi claude --hapi-starting-mode remote --started-by runner
        │   ├─ HAPI MCP Server (HTTP, 127.0.0.1:随机端口)
...
| 进程 | 终端归属 | stdio 配置 | 说明 |
|------|---------|-----------|------|
| `hapi runner start` | 用户终端 | 继承 | 立即退出 |
...
| HAPI MCP Server | 无（后台） | N/A | HTTP 服务器，无 stdio |
    ...
    "SessionStart": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "hapi hook-forwarder --port <PORT> --token <TOKEN>"
          }
        ]
      }
    ]
...
2. 触发 SessionStart hook，执行 `hapi hook-forwarder`
...
**连接建立：**
- Runner 启动时连接到 HAPI Hub（`configuration.apiUrl`）
...
**创建时机：**
- 用户通过 HAPI Hub 请求创建 worktree 会话
...
| `cli/src/claude/utils/startHappyServer.ts` | HAPI HTTP MCP 服务器 |
| `cli/src/claude/utils/startHookServer.ts` | Session Hook HTTP 服务器 |
| `cli/src/codex/happyMcpStdioBridge.ts` | STDIO → HTTP MCP 桥接 |
| `cli/src/modules/common/hooks/generateHookSettings.ts` | Hook 配置文件生成 |
| `cli/src/utils/spawnHappyCLI.ts` | 跨平台 HAPI CLI 进程启动工具 |
...
- 代码中所有 `spawnHappyCLI` 调用都使用了 `detached: true`，但没有设置 `windowsHide: true`
...
**修复方案：**
- 修改 `cli/src/utils/spawnHappyCLI.ts` 的 `spawnHappyCLI` 函数
- 在 Windows 平台且 `detached: true` 时，自动注入 `windowsHide: true`
- 这样所有调用处都自动生效，无需逐个修改

**修复代码：**
```typescript
// cli/src/utils/spawnHappyCLI.ts:108-114
...
在不依赖手工 GUI 观察的前提下，为 `spawnHappyCLI` 的 Windows 修复建立可复用、命令行可执行的验证路径，并实际执行可在当前环境完成的验证。

### What I already know

- 当前改动点是 `cli/src/utils/spawnHappyCLI.ts:108-114`，在 `win32 + detached` 场景注入 `windowsHide: true`。
...
- How it works:
  - 新增 `spawnHappyCLI` 单元测试，mock `child_process.spawn`，断言：
...
- How it works:
  - 执行 `hapi runner start` / `hapi runner status` / `hapi runner stop`，配合日志断言无异常
- Pros:
  - 更接近真实启动链路
- Cons:
...
- [x] 存在针对 `spawnHappyCLI` 的单元测试，覆盖 `windowsHide` 注入逻辑。
...
- 候选测试文件：`cli/src/utils/spawnHappyCLI.test.ts`（新建）。
- 参考现有测试风格：`cli/src/claude/utils/path.test.ts`（vitest + mock）。
- 相关代码位置：
  - `cli/src/utils/spawnHappyCLI.ts:80-115`
...
1. 新增测试文件：`cli/src/utils/spawnHappyCLI.test.ts`
   - 覆盖场景：
...
2. 命令执行：
   - `cd "C:/Users/joey/code/hapi/cli" && bunx vitest run src/utils/spawnHappyCLI.test.ts`
   - 结果：1 file passed, 3 tests passed

3. 类型检查：
   - `cd "C:/Users/joey/code/hapi/cli" && bun run typecheck`
